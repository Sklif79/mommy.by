...
$(document).ready(function () {
	...
	setTimeout(function () {
		...
		// $('#menu-flex-scroll > .flexMenu-viewMore > a').text('Каталог товаров');
	...
	//ввел свою опцию left в плагин jquery.ez-plus.js (позиция слева от изображения фикс)
	if ($(window).width() >= 1024) {
		$('.main-first-image-slider-el img').ezPlus({
			responsive: true,
			scrollZoom: false,
			showLens: true,
			borderSize: 1,
			zoomWindowHeight: 339,
			zoomWindowWidth: 340,
	...
	//zoom
	// $('.main-first-image-slider-el').easyZoom({
	//     width: 250,
	//     position: 'right',
	//     background: '#222',
	// });
	...
	if ($(window).width() > 480) {
		$("form").on('click', '.placeholder_custom', function () {
			$(this).removeClass('active');
			$(this).prev().focus();
		});

		$("form").on('focusout', 'input, textarea, .js_custom-input', function () {
			if ($(this).val().trim() == '') {
